...
double* AppPottsOri::read_shockley_table() {
  int table_size = (nspins+1) + (nspins* (nspins+1)) / 2;
  double* table = NULL;
  table = new double[table_size];
...
void AppPottsOri::compute_misorientation_angles(std::string symmetry_filename) {
...
Quaternion<double> AppPottsOri::misori(Quaternion<double> ori_a, Quaternion<double> ori_b, int N_symm, double *symm) {
...
bool AppPottsOri::cubic_FZ_test(Quaternion<double> quat) {
...
double *AppPottsOri::load_symm_table(int &N_symm, std::string infile_name) {
...
double *AppPottsOri::load_euler_orientations_as_quats(char *filename) {
  std::string line;
  std::ifstream infile(filename);
  if (!infile.is_open()) error->all(FLERR,"Could not open lookup table file");
  std::getline(infile, line); // ignore the first line
...
void AppPottsOri::quat_from_Bunge(double phi_1, double Phi, double phi_2, double *xyzw) {
